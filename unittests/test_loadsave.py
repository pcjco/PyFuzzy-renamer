...
class loadsave_Tests(pfr.PyFuzzyRenamerTestCase):
    ...
    def test_loadsave(self):
        ...
        item = -1
        item = lst.GetNextItem(item)
        self.assertEqual(["Acanthe à feuilles molles.txt", "", "", "", "No match", "True"], [lst.GetItemText(item, col) for col in range(0, len(pyfuzzyrenamer.default_columns))])
        item = lst.GetNextItem(item)
        self.assertEqual(["Acanthe épineuse.txt", "", "", "", "No match", "True"], [lst.GetItemText(item, col) for col in range(0, len(pyfuzzyrenamer.default_columns))])
        item = lst.GetNextItem(item)
        ...
        item = lst.GetNextItem(item)
        self.assertEqual(["Aconit vénéneux.txt", "52", "Aconitum anthora.txt", "Aconitum anthora.txt", "Matched", "False"], [lst.GetItemText(item, col) for col in range(0, len(pyfuzzyrenamer.default_columns))])
        item = lst.GetNextItem(item)
